...
#python make_ngram_model.py final_lnnt.txt 1 > unigram_model.txt

#python make_ngram_model.py final_lnnt.txt 2 > bigram_model.txt
...
paste count_capital.txt count_emoticon_class.txt fv_pos_count.txt fv_punct_and_elong.txt -d " " > fv_3456789.txt


#let d=$c+$x ##used when trigram is being used.
let d=$c ## used when tri gram is not being used
