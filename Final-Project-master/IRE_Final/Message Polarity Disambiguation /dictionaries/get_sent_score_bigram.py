...
def f1_lookup_and_compute(seg, dit):
	...
	if lg >= 2:
		#for last token
		if " ".join(temp[-2:]) in dit:
			score_lt4 = dit[" ".join(temp[-2:])]

		#for first token
		if " ".join(temp[:2]) in dit:
			score_ft5 = dit[" ".join(temp[:2])]

	if lg == 2:
		score_ft5 = 0.0  #if tweet is only of length 2

	for k in range(0, lg-1):
		i = " ".join(temp[k:k+2])
		if i in dit:
			if dit[i] > 0:
				score_gt1 += dit[i]
...
def main():
	...
	fn2 = './NRC_hashtag/bigrams-pmilexicon.txt'
	fn3 = './sentiment140_lexicon/bigrams-pmilexicon.txt'
